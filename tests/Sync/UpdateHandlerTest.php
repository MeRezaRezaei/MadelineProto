...
class UpdateHandlerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->emitted = [];
        $emittedRef = &$this->emitted;

        // Spy EventBus: records emit calls without network.
        $bus = new class($emittedRef) extends EventBus {
            public function __construct(private array &$emittedList)
            {
            }
            public function emit(int $accountId, string $type, array $data): void
            {
                $this->emittedList[] = [$accountId, $type, $data];
            }
        };
